...
exports.createCarUser = [
  ...
  body("profession")
    ...
    .withMessage("Profession must be one of 'IT', 'Sales', or 'Unemployed'."),
  body("password")
    .isLength({ min: 6 })
  ...
  async function (req, res, next) {
    ...
    if (errors.isEmpty()) {
      let { username, email, age, gender, dob, city, profession, password } =
        req.body;
      let user = new CarRentalUser({
        username,
        email,
        age,
        gender,
...
exports.getCarUser = async function (req, res, next) {
  let results = await CarRentalUser.find();
  res.json(results);
};

exports.getCarUserById = async function (req, res, next) {
...
exports.putCarUser = [
  ...
  async function (req, res, next) {
    ...
    if (errors.isEmpty()) {
      ...
      let result = await CarRentalUser.findByIdAndUpdate(
        req.params.id,
        updateData,
        { new: true }
      );
...
exports.deleteCarUser = async function (req, res, next) {
  let id = req.params.id;
