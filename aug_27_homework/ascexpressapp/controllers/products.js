...
exports.createProduct =
 [
    ...
    body("availability").isIn(['Available', 'Not Available']).withMessage("Availability must be one of 'Available', 'Not Available'"),
...
exports.putProduct = [
    ...
    async function (req, res, next) {
        ...
        if (errors.isEmpty()) {
            ...
            let result = await Product.findByIdAndUpdate(req.params.id, updateData, { new: true });
            res.json(result);
        } else {
            res.status(400).json(errors.array());
        }
